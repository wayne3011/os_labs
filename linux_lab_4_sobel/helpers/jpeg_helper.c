...
decompressed_jpeg applySobelFilter(decompressed_jpeg *image,
  ...
  decompressedJpegResutlt.imageData = (unsigned char **)malloc(
      (processedHeightEnd - processedHeightStart) * sizeof(unsigned char *));

  for (int i = processedHeightStart, s = 0; i < processedHeightEnd; i++, s++) {
    decompressedJpegResutlt.imageData[s] =
        (unsigned char *)malloc(image->width * 3);
    for (int j = 0; j < image->width; j += 1) {
      ...
      decompressedJpegResutlt.imageData[s][j * 3] =
          decompressedJpegResutlt.imageData[s][j * 3 + 1] =
              decompressedJpegResutlt.imageData[s][j * 3 + 2] = magnitude;
    }
  }
...
int readJPEG(FILE *infile, decompressed_jpeg *decompressedImage) {
  ...
  decompressedImage->imageData = (unsigned char **)malloc(
      (decompressedImage->height) * sizeof(unsigned char *));
  for (int i = 0; i < decompressedImage->height; i++) {
    (decompressedImage->imageData)[i] =
        (unsigned char *)malloc((decompressedImage->width) * 3);
  }
